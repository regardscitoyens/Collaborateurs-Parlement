...
tac data/turnover-collab.commits.csv.tmp | while read COMMIT ; do
	...
	git diff $COMMIT $PREVIOUSCOMMIT data/liste_deputes_collaborateurs.csv | grep '^[-+]' | sed 's/^\+/"AJOUT",/' | sed 's/^-/"SUPPRESSION",/' | sed "s/$/,\"$DATECOMMIT\"/" > data/tunover-collab.ajoutsuppression.csv.tmp
done

python bin/collab_mouvements.py data/tunover-collab.ajoutsuppression.csv.tmp
